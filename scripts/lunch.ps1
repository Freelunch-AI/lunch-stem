...
EXAMPLES:
    ...
    .\lunch.ps1 folder "C:\project\target" --recursive --in-place
...
function Download-DvcFile {
    ...
    try {
        # Download file using rclone (suppress output like bash version)
...
function Invoke-DirectoryCommand {
    ...
    # Reconstruct the directory path by joining non-flag arguments
    $pathParts = @()
    $i = 0
